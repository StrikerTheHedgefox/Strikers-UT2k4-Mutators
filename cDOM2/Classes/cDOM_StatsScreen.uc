//==============================================================================
// Add cDOM specific game stats to the players PERSONAL STATS screen (F3 ingame)
//==============================================================================
class cDOM_StatsScreen extends DMStatsScreen;

var localized string DomPointCapture, GoalResults;
...
simulated event DrawScoreboard( Canvas C )
{
	...
	if ( PRI.FlagReturns > 0 )
		GoalsNum++;
	if ( cDOM_PlayerReplicationInfo(PRI).DomPointCapture > 0 )
		GoalsNum++;
	if ( GoalsNum > 0 )
	{
		C.DrawColor = HUDClass.default.GoldColor;
		GoalsBoxSizeY = LargeYL + GoalsNum + 4 * LargeYL;
		C.SetPos(3*IndentX+CombatBoxX, GoalsOffsetY);
		C.StrLen(GoalResults$" 999 XXXxxXX", XL, LargeYL);
		GoalsBoxX = XL + 3*IndentX;
	...
	if ( GoalsNum > 0 )
	{
		...
		if ( cDOM_PlayerReplicationInfo(PRI).DomPointCapture > 0 )
		{
			C.DrawText(DomPointCapture);
			C.StrLen(cDOM_PlayerReplicationInfo(PRI).DomPointCapture, XL, LargeYL);
			C.SetPos(IndentX+CombatBoxX+GoalsBoxX - XL,OffsetY);
			C.DrawText(cDOM_PlayerReplicationInfo(PRI).DomPointCapture);
			OffsetY += LargeYL;
			C.SetPos(4*IndentX+CombatBoxX,OffsetY);
		}
		//  </DomPointCaptures>
...
defaultproperties
{
	DomPointCapture="Control Point Captures:"
	GoalResults="CONTROL POINT STATS"
}
