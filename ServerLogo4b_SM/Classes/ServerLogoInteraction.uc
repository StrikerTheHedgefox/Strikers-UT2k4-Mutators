...
class ServerLogoInteraction extends Interaction
    dependson(ServerLogo);
...
var config ServerLogo.EFadeTransition TestTransition;
...
function DrawScreenTexture(Canvas C, Material M, float X, float Y, float W, float H,
    ServerLogo.TTexRegion R, EDrawPivot Pivot)
{
  local float ratio;
...
function float FadeIn(float Alpha, float Start, float End, ServerLogo.EFadeTransition Method)
{
  switch (Method) {
  Case FT_None:
    return End;
...
function float FadeOut(float Alpha, float Start, float End, ServerLogo.EFadeTransition Method)
{
  if ( Method == FT_None )
    return Start;
  else
...
defaultproperties
{
	Begin Object Class=TexRotator Name=LogoRotator
        TexRotationType=TR_ConstantlyRotating
		TexCoordCount=TCN_2DCoords
		TexCoordProjected=False
    End Object
	
	RotatingLogoMaterial=TexRotator'LogoRotator'
	bVisible=True
}